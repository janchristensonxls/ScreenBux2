...
/// <summary>
/// Bridges the Service's on-demand "capture all screens" flow (triggered asynchronously via
...
/// receives; when the Agent's follow-up <see cref="Messages.ScreenCaptureReportMessage"/> arrives,
/// it completes the corresponding waiter. Mirrors <see cref="PendingWindowListRequestCoordinator"/>.
/// </summary>
public class PendingScreenCaptureRequestCoordinator
{
    private readonly ConcurrentDictionary<Guid, TaskCompletionSource<List<CapturedImage>?>> _pending = new();
    ...
    {
        _pending.TryAdd(requestId, new TaskCompletionSource<List<CapturedImage>?>(TaskCreationOptions.RunContinuationsAsynchronously));
    }

    /// <summary>
    /// Awaits the result of a previously <see cref="Register"/>-ed request, or times out.
    /// </summary>
    public async Task<List<CapturedImage>?> WaitAsync(Guid requestId, TimeSpan timeout, CancellationToken cancellationToken)
    {
    ...
    /// <summary>
    /// Called when the Agent's <see cref="Messages.ScreenCaptureReportMessage"/> arrives.
    /// </summary>
    public void Complete(Guid requestId, List<CapturedImage>? images)
    {
        if (_pending.TryGetValue(requestId, out var tcs))
        {
